...
def _schemas_dir() -> Path:
    # apps/edge-ingest/edge_ingest/schema_validation.py -> repo/apps/cortai-api/schemas
    return Path(__file__).resolve().parents[2] / "cortai-api" / "schemas"
...
def validate_edge_envelope(message: Any) -> None:
    ...
    if errors:
        logger.warning("edge.envelope.invalid", error_count=len(errors), errors=errors)

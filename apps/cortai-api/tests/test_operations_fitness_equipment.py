...
async def seeded_property_for_fitness_equipment() -> dict[str, uuid.UUID]:
    org_id = uuid.uuid4()
    now = datetime.now(UTC)
    prop_id = uuid.uuid4()

    async with SessionLocal() as session:
        session.add(Organization(id=org_id, name="Fitness Equipment Org", slug=f"fitness-eq-{org_id}"))
...
@pytest.mark.asyncio
async def test_fitness_equipment_returns_fixture_payload(seeded_property_for_fitness_equipment) -> None:  # type: ignore[no-untyped-def]
    org_id = seeded_property_for_fitness_equipment["org_id"]
    prop_id = seeded_property_for_fitness_equipment["property_id"]
    async with _client_for_org(org_id=org_id) as client:
        resp = await client.get(f"/api/operations/fitness/equipment?property_id={prop_id}")
...
@pytest.mark.asyncio
async def test_fitness_equipment_404_when_property_missing(seeded_property_for_fitness_equipment) -> None:  # type: ignore[no-untyped-def]
    org_id = seeded_property_for_fitness_equipment["org_id"]
    missing = uuid.uuid4()
    async with _client_for_org(org_id=org_id) as client:
        resp = await client.get(f"/api/operations/fitness/equipment?property_id={missing}")

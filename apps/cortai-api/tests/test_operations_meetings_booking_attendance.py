...
async def seeded_booking_for_attendance() -> dict[str, uuid.UUID]:
    org_id = uuid.uuid4()
    now = datetime.now(UTC)
    prop_id = uuid.uuid4()

    async with SessionLocal() as session:
        session.add(Organization(id=org_id, name="Meetings Attendance Org", slug=f"meetings-att-{org_id}"))
...
@pytest.mark.asyncio
async def test_meeting_booking_attendance_returns_fixture(seeded_booking_for_attendance) -> None:  # type: ignore[no-untyped-def]
    org_id = seeded_booking_for_attendance["org_id"]
    booking_id = seeded_booking_for_attendance["booking_id"]
    async with _client_for_org(org_id=org_id) as client:
        resp = await client.get(f"/api/operations/meetings/bookings/{booking_id}/attendance")
...
@pytest.mark.asyncio
async def test_meeting_booking_attendance_404_when_missing(seeded_booking_for_attendance) -> None:  # type: ignore[no-untyped-def]
    org_id = seeded_booking_for_attendance["org_id"]
    missing = uuid.uuid4()
    async with _client_for_org(org_id=org_id) as client:
        resp = await client.get(f"/api/operations/meetings/bookings/{missing}/attendance")

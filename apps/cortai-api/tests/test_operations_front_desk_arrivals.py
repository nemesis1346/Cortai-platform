...
async def seeded_arrivals() -> dict[str, uuid.UUID]:
    org_id = uuid.uuid4()
    ...
    r_cancelled = uuid.uuid4()

    async with SessionLocal() as session:
        session.add(Organization(id=org_id, name="FD Arrivals Org", slug=f"fd-arrivals-{org_id}"))
    ...
    async with SessionLocal() as session:
        ...
        await session.execute(
            text(
                ...
                values
                  (:rvip, :org, :gvip, :p1, null, 'booked',
                   :in1, :out1, 10000, null, 'pms', :now, :now),
                  (:rnorm, :org, :gn, :p1, null, 'booked',
                   :in2, :out2, 10000, null, 'pms', :now, :now),
                  (:rother, :org, :gn, :p2, null, 'booked',
                   :in1, :out1, 10000, null, 'pms', :now, :now),
                  (:rcancel, :org, :gn, :p1, null, 'cancelled',
                   :in1, :out1, 10000, null, 'pms', :now, :now)
                """
            ),
            {
                ...
                "p1": prop_a,
                "p2": prop_b,
                "in1": today_start + timedelta(hours=12),
                "out1": today_start + timedelta(days=1),
                "in2": today_start + timedelta(hours=13),
                "out2": today_start + timedelta(days=1),
                "now": now,
            },
        )
        await session.commit()
...
@pytest.mark.asyncio
async def test_front_desk_arrivals_filters_by_property_and_date(seeded_arrivals) -> None:  # type: ignore[no-untyped-def]
    org_id = seeded_arrivals["org_id"]
    prop_id = seeded_arrivals["property_id"]
    today = datetime.now(UTC).date()

    async with _client_for_org(org_id=org_id) as client:
        resp = await client.get(f"/api/operations/front-desk/arrivals?property_id={prop_id}&date={today.isoformat()}")
    assert resp.status_code == 200
    body = resp.json()
    assert body["date"] == today.isoformat()
    # VIP should be listed first

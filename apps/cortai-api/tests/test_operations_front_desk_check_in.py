...
async def seeded_check_in() -> dict[str, uuid.UUID]:
    org_id = uuid.uuid4()
    ...
    async with SessionLocal() as session:
        session.add(Organization(id=org_id, name="FD CheckIn Org", slug=f"fd-checkin-{org_id}"))
...
@pytest.mark.asyncio
async def test_front_desk_check_in_assigns_room_updates_room_and_creates_key_request(seeded_check_in) -> None:  # type: ignore[no-untyped-def]
    org_id = seeded_check_in["org_id"]
    reservation_id = seeded_check_in["reservation_id"]
    room_id = seeded_check_in["room_id"]

    async with _client_for_org(org_id=org_id) as client:
        resp = await client.post(f"/api/operations/front-desk/check-in/{reservation_id}", json={"room_id": str(room_id)})
    ...
    async with SessionLocal() as session:
        ...
        res = (
            await session.execute(
                text("select status, room_id from ops.reservations where id = :id and org_id = :org"),
